// src/pipeline.ts
import { DataLoader, Document } from './data-loader';
...
export class RAGPipeline {
  ...
  async query(query: string): Promise<LLMResponse> {
    ...
    if (relevantDocuments.length === 0) {
      ...
      // Fallback strategy: query LLM directly or return a specific message
      // For now, let's try to answer with LLM without context
      // return this.llm.generate(`Could not find relevant context. Please answer based on general knowledge: ${query}`);
      return {
